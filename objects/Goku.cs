...
public partial class Goku : CharacterBody2D
{
	...
	private CollidersScriptG _collidersScript ;
	...
	public override void _Ready(){
		...
		_collidersScript = GetNode<CollidersScriptG>("Colliders");
		...
		deadMessage = GetNode<Label>("DeadGoku");
		...
		_collidersScript.HandleAllColliderDisabling(true);
		 // Configurar capas y máscaras de colisión
	...
	public override void _PhysicsProcess(double delta)
	{
		...
		// Handle Jump.
		if (Input.IsActionJustPressed("move_upG") && IsOnFloor())
		{
		...
		Vector2 direction = Input.GetVector("move_leftG", "move_rightG", "move_upG", "move_downG");
		...
		if (Input.IsActionJustPressed("punchG"))
		{
			PerformAttack("punch");
		}
		if (Input.IsActionJustPressed("kickG"))
		{
			PerformAttack("kick");
		}
		if (Input.IsActionJustPressed("kamehameha"))
		{
			PerformAttack("kamehameha");
		}
	}
	 private async void PerformAttack(string attackType)
	{
		...
		switch (attackType)
		{
			...
			case "kamehameha":
				...
				if(isFacingLeft){
					_collidersScript.UpdateColliderPosition("LeftHandCollider", new Vector2(-100, 10));
					_collidersScript.UpdateColliderPosition("RightHandCollider", new Vector2(-10, 0)); 
					
					_collidersScript.UpdateColliderPosition("LeftFootCollider", new Vector2(-50, -10)); 
					_collidersScript.UpdateColliderPosition("RightFootCollider", new Vector2(-50, 0)); 
				}else{
					_collidersScript.UpdateColliderPosition("LeftHandCollider", new Vector2(90, 0));
					_collidersScript.UpdateColliderPosition("RightHandCollider", new Vector2(70, 0)); 
					
					_collidersScript.UpdateColliderPosition("LeftFootCollider", new Vector2(60, -20)); 
					_collidersScript.UpdateColliderPosition("RightFootCollider", new Vector2(40, -20)); 
				}
	...
	public void TakeDamage(int damage){
		...
		GD.Print("Goku ha recibido daño");
	}
	
	
	public void Dead(){
		GD.Print("Goku ha muerto");
		isAlive = false;
